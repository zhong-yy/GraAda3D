...
AdaptiveInversion::AdaptiveInversion(const Mesh& mesh_,
...
#pragma omp parallel for
      for (int j = 0; j < 8; j++) {
        ...
        gra.field_caused_by_single_prism(ob(i), *(parent_cell->child_cells[j]),
    ...
    for (int j = 0; j < 2; j++) {
      ...
      if (fx->isleaf) {
        ...
        for (int k = 0; k < 4; k++) {
          ...
          int id = neigh->get_id();
          double area =
              (neigh->_z[1] - neigh->_z[0]) * (neigh->_y[1] - neigh->_y[0]);
          indicator(i) += (m(id) - mc) * (m(id) - mc) * area;
          // double vol = neigh->get_volumn();
          // indicator(i) += (m(id) - mc) * (m(id) - mc) * vol;
        }
      ...
      if (fy->isleaf) {
        ...
        for (int k = 0; k < 4; k++) {
          ...
          int id = neigh->get_id();
          double area =
              (neigh->_z[1] - neigh->_z[0]) * (neigh->_x[1] - neigh->_x[0]);
          indicator(i) += (m(id) - mc) * (m(id) - mc) * area;
          // double vol = neigh->get_volumn();
          // indicator(i) += (m(id) - mc) * (m(id) - mc) * vol;
        }
      ...
      if (fz->isleaf) {
        ...
        for (int k = 0; k < 4; k++) {
          ...
          int id = neigh->get_id();
          double area =
              (neigh->_x[1] - neigh->_x[0]) * (neigh->_y[1] - neigh->_y[0]);
          indicator(i) += (m(id) - mc) * (m(id) - mc) * area;
        }
      }
    }
...
void AdaptiveInversion::refine_mesh(double a,
  ...
  // indicator = ((D_theta1 * m).cwiseAbs2() + (D_phi1 * m).cwiseAbs2() + (D_r1
  ...
  for (int i = 0; i < Nm; i++) {
    if (indicator(i) > tol) {
      ...
      {
        // If the size of the cell is less than the limit of minimum size,
        // do nothing
      } else {
  ...
  for (int i = 0; i < cells_marked.size(); i++) {
    if (cells_marked[i]->isleaf) {
      map<unsigned int, Cell*> split_cells0 = mesh.refinement(cells_marked[i]);
    ...
    for (int i = 0; i < split_cells.size(); i++) {
      ...
      for (int j = 0; j < 8; j++) {
        ...
        if (flag == "crg") {
          ...
          cout << "flag must be \"crg\" or \"pet\" or\"both\"" << endl;
          abort();
        }
      }
      map_it++;
