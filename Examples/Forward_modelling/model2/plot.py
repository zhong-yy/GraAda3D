...
for fname,outfile_remark in zip(['result/data_no_noise','result/data'],["","_with_noise"]):
    ...
    # Txz=Txz-Txz2
    # Tyz=Tyz-Tyz2
    X=np.reshape(x,(61,61),order='C')
    Y=np.reshape(y,(61,61),order='C')
    GZ=np.reshape(gz,(61,61),order='C')
    TZZ=np.reshape(Tzz,(61,61),order='C')
    TXZ=np.reshape(Txz,(61,61),order='C')
    TYZ=np.reshape(Tyz,(61,61),order='C')


    fig,ax1=plt.subplots(1,1,figsize=(6,4))    

    pc=ax1.pcolormesh(X,Y,GZ,shading='gouraud',cmap=clrmap)#vmin,vmax 设置色标的上下限
    ax1.contour(X,Y,GZ,levels=np.arange(-1,3.5,0.5),colors='k')
