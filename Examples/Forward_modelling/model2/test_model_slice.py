...
mydata=Dataset('model/test_model2.nc', "r")# r mean read
print(mydata)
...
z_id=get_slice_index(zs,1250)
z_slice=density[z_id,:,:]

x_id=get_slice_index(xs,3000)
x_slice=density[:,:,x_id]

y_id=get_slice_index(ys,3000)
y_slice=density[:,y_id,:]
...
pc=ax1.pcolormesh(Y,Z,x_slice,cmap=clrmap,vmin=min_value_shown,vmax=max_value_shown)
...
pc=ax2.pcolormesh(X,Z,y_slice,cmap=clrmap,vmin=min_value_shown,vmax=max_value_shown)
...
ax3.text(0.01,0.92,"(c) Z=1250 m",fontsize=fts, transform=ax3.transAxes)
